...
export class GroupCommand {
  ...
  private disableCmd$ = this.store.select(selectNumberOfSelectedShapes).pipe(map((n) => n < 2));

  private item = this.mb.commandItem({
    label: 'Group',
    subLabel: 'Ctrl+G',
    disabled$: this.disableCmd$,
    exec: this.doGroup(),
  });
  ...
  getItem(): CommandItem {
    ...
    this.hotkey = new Hotkey('ctrl+g', () => {
      if (this.selectedShapeIds.length > 1) {
        this.doGroup()(this.item);
      }
      return false;
    });
  ...
  private doGroup(): (commandItem: CommandItem) => void {
    return () => {
      this.store.dispatch(
        ArrangeMenuActions.groupClick({ selectedShapeIds: this.selectedShapeIds })
      );
    };
  }
}

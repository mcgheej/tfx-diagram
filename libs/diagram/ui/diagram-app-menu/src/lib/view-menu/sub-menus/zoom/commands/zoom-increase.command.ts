...
export class ZoomIncreaseCommand extends ZoomCommandBase {
  private hotkey: Hotkey | null = null;

  private item = this.mb.commandItem({
    label: 'Zoom In',
    subLabel: 'Ctrl+=',
    exec: this.execIncreaseZoom(),
  });
  ...
  getItem(): CommandItem {
    ...
    this.hotkey = new Hotkey('ctrl+=', () => {
      this.execIncreaseZoom()(this.item);
      return false;
    });
  ...
  private execIncreaseZoom(): (commandItem: CommandItem) => void {
    return () => {
      this.store.dispatch(

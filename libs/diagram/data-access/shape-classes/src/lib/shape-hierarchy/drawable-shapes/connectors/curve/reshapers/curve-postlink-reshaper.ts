...
export class CurvePostlinkReshaper extends CurveNonEndpointReshaper {
  modifiedShape(
    newHandlePos: Point,
  ...
  private modify(
    ...
    modifiedShapes.push(
      (controlFrame[2 * nSegments + cpIdx] as Handle).copy({
        x: hPos.x,
        y: hPos.y,
      })
    );
    ...
    modifiedShapes.push(
      (controlFrame[2 * nSegments + cpIdx - 2] as Handle).copy({
        x: hPos.x,
        y: hPos.y,
      })
    );
    ...
    let midPoint = calcBezierPoint(cpIdx - 3, cp, 0.5);
    modifiedShapes.push(
    ...
    midPoint = calcBezierPoint(cpIdx, cp, 0.5);
    modifiedShapes.push(

...
export class RectangleEReshaper extends RectReshaper {
  modifiedShape(
    newHandlePos: Point,
    ...
    const newRect = this.eReshape(newPos, {
  ...
  private eReshape(newPos: Point, rect: Rect): Rect {
    const { left, top, bottom } = rectEdges(rect);

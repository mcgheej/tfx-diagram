...
export class RectangleNReshaper extends RectReshaper {
  modifiedShape(
    newHandlePos: Point,
    ...
    const newRect = this.nReshape(newPos, {
  ...
  private nReshape(newPos: Point, rect: Rect): Rect {
    const { left, right, bottom } = rectEdges(rect);
